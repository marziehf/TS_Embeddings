...
real *syn0, *syn0Double, *syn1, *syn1neg, *expTable;
clock_t start;
int numiters = 1;
...
void InitNet(struct vocabulary *wv, struct vocabulary *cv) {
    ...
    z = posix_memalign((void **)&syn0Double, 128, (long long)cv->vocab_size * layer1_size * sizeof(real));
    if (syn1neg == NULL) {printf("Memory allocation failed\n"); exit(1);}
    if (syn0Double == NULL) {printf("Memory allocation failed\n"); exit(1);}
    for (b = 0; b < layer1_size; b++) {
        for (a = 0; a < cv->vocab_size; a++) {
            syn1neg[a * layer1_size + b] = 0;
        }
        for (z = 0; z < cv->vocab_size; z++) {
            syn0Double[z * layer1_size + b] = (rand() / (real)RAND_MAX - 0.5) / layer1_size;
        }
    }
}
...
void *TrainModelThread(void *id) {
    ...
    real f, g;
    clock_t now;
    real *neu1 = (real *)calloc(layer1_size, sizeof(real)); //neu1: hidden layer values
    real *neu1e = (real *)calloc(layer1_size, sizeof(real));
    char *tmpw = (char *)calloc(1000, sizeof(char));
    int prev_word_id = wv->vocab_size + 100;
    ...
    for (iter=0; iter < numiters; ++iter) {
        ...
        while (1) { //HERE @@@
            ...
            if(wrdi != prev_word_id) {
                char * token_sep = strstr(wv->vocab[wrdi].word, "___");
                if(token_sep != NULL) {
                    //tmpws = strdup(wv->vocab[wrdi].word);
             //       printf("in if cond.\n");
                    //char *token = strtok(tmpws, "___");
                    //printf("after strtok: %s\n", tmpws);
                    //wv->vocab[wrdi].word = tmpws;
                    int sep_id = token_sep - wv->vocab[wrdi].word;
                    strncpy(tmpw, wv->vocab[wrdi].word, sep_id);
                    tmpw[sep_id] = '\0';
                    l_tmp = SearchVocab(cv, tmpw);                
                    //   printf("%s\n", wv->vocab[wrdi].word);
                    //  printf("%s\n", tmpw);
                }
                else {
                    l_tmp = SearchVocab(cv, wv->vocab[wrdi].word);
                }
            }

            if(l_tmp >= 0) {
                for (c = 0; c < layer1_size; c++) syn0[c + l1] = syn0[c + l1] + syn0Double[c + l_tmp];
                changed = 1;
            }

//            printf("%s\n", wv->vocab[wrdi].word);
            //printf("tmpw and l_tmp: %s %ld\n", tmpw, l_tmp);
            //printf("%d\n\n", changed);
            //        free(token);
            //              printf("after: %s\n", wv->vocab[wrdi].word);
            ...
            for (d = 0; d < negative + 1; d++) {
                if (d == 0) {
                    target = ctxi;
                    label = 1; //moi: label means this is TARGET and not NEG SAMP!
                } else {
                    next_random = next_random * (unsigned long long)25214903917 + 11;
                ...
                if (f > MAX_EXP) g = (label - 1) * alpha; //gradient = 0 for target and -alpha for negative samples
                else if (f < -MAX_EXP) g = (label - 0) * alpha; //same ^
                else g = (label - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha; //g: gradient. error of output.
                //printf("g: %f \n", g);
                for (c = 0; c < layer1_size; c++) {
                    neu1e[c] += g * syn1neg[c + l2]; //backpropagate error to hidden layer. saves sum for all negative samples to update input2hidden later
                    //printf("syn1neg: %f ", syn1neg[c + l2]);
                }
                for (c = 0; c < layer1_size; c++) syn1neg[c + l2] += g * syn0[c + l1]; //adding error back to word
            }
            // Learn weights input -> hidden
            //for (c = 0; c < layer1_size; c++) syn0[c + l1] += neu1e[c];
            // update the original vector and not w+ws
            if(changed == 1) {
                for (c = 0; c < layer1_size; c++) {
                    syn0[c + l1] = syn0[c + l1] - syn0Double[c + l_tmp];
                }
            }
            for (c = 0; c < layer1_size; c++) {
                syn0[c + l1] += neu1e[c];
                syn0Double[c + l_tmp] += neu1e[c];
             //   printf("neue: %s ", neu1e[c]);
            }
            //printf("Done.\nneue: ");
        
       //     for (c = 0; c < layer1_size; c++) printf("%f ", neu1e[c]);
         //   printf("\nsyn0: ");
           // for (c = 0; c < layer1_size; c++) printf("%f ", syn0[c + l1]);
            //printf("\nsyn0double: ");
            //for (c = 0; c < layer1_size; c++) printf("%f ", syn0Double[c + l_tmp]);
            //printf("\n");
        //for (c = 0; c < layer1_size; c++) printf("syn1neg: %s ", syn1neg[c + l2]);
        //printf("\n");
            //printf("\n");
        }
    }
...
void TrainModel() {
    ...
    if (classes == 0) {
        ...
        if (dumpcv_file[0] != 0) {
            ...
            fo3 = fopen(dumpcv_file2, "wb");
            fprintf(fo3, "%d %d\n", cv->vocab_size, layer1_size);
            for (a = 0; a < cv->vocab_size; a++) {
                fprintf(fo3, "%s ", cv->vocab[a].word); //TODO
                if (binary) for (b = 0; b < layer1_size; b++) fwrite(&syn0Double[a * layer1_size + b], sizeof(real), 1, fo3);
                else for (b = 0; b < layer1_size; b++) fprintf(fo3, "%lf ", syn0Double[a * layer1_size + b]);
                fprintf(fo3, "\n");
            }
        }
...
int main(int argc, char **argv) {
    ...
    if ((i = ArgPos((char *)"-dumpcv2", argc, argv)) > 0) strcpy(dumpcv_file2, argv[i + 1]);
